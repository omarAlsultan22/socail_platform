...
class _PostCardState extends State<PostCard> with SingleTickerProviderStateMixin {
  ...
  void addAnimationValue() {
    if (widget.postModel!.userId == widget.sessionService.currentUid) {
      setState(() {
        value = 70.0;
      });
    }
  ...
  void userProfile(MainLayoutCubit cubit) {
    ...
    else if (widget.postModel!.userId != widget.sessionService.currentUid) {
      BuildNavigator.build(context: context,
          link: FriendProfileScreen(userId: widget.postModel!.userId!));
    }
    else {
          ...
          child: Column(
            children: [
              animation.value == value &&
                  widget.postModel!.userId == widget.sessionService.currentUid ?
              SmallMenuWidget(
                  context: context,
                  buttonName: 'Edit',
                  onPressed: () async {
                ...
                onPressed: () async {
                  await ExitDialogHelper.showExitDialog(
                      ...
                      onPressed: (val) {
                        if (!val) return;
                        if (widget.postModel!.userId == widget.sessionService.currentUid) {
                        _profileCubit.deletePost(postModel: widget.postModel);
                        }
                        _publicCubit.deletePost(postModel:widget.postModel);
                      },
                      type: 'post'
                  );
                  _animationController();
